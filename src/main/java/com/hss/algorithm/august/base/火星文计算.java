...
public class 火星文计算 {
    public static void main(String[] args) {
        ...
        StringBuilder str = new StringBuilder();    //用来保存数字字符串
        for (int i = 0; i < s.length(); i++) {
            if (Character.isDigit(s.charAt(i))) {
                str.append(s.charAt(i));   //考虑到多位数需要拼接
                if (i == s.length() - 1) {    //最后一个字符
                    int n = Integer.parseInt(str.toString());   //最后一个肯定是数字
                    char f = fuhao.peek();
                    if (f == '#') {
                ...
                if (fuhao.isEmpty()) {
                    num.push(Integer.valueOf(str.toString())); //上个str数字化push数字堆栈
                    str = new StringBuilder();   //重置str
                    fuhao.push(s.charAt(i));    //本次字符串push符合堆栈
                } else {
                    int n = Integer.parseInt(str.toString());   //str数字化
